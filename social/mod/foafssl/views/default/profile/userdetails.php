...
<td>
	...
	<div xmlns:foaf="http://xmlns.com/foaf/0.1/" xmlns:cert="http://www.w3.org/ns/auth/cert#" xmlns:rsa="http://www.w3.org/ns/auth/rsa#" id="profile_info_column_middle" >
			<?php
	...
	echo "<h2><a about='#me' typeof='foaf:Person' href=\"" . $vars['entity']->getUrl() . "\" rel=\"$rel\">" . $vars['entity']->name . "</a></h2>";
					...
					if (!empty($value)) {
						...
						<p class="<?php echo $even_odd; ?>">
							...
							//echo elgg_view("output/{$valtype}", $options);
                                                 $property = ($shortname == 'publickeymodulus')? ' property=modulus ':'';
                                                 if ( $shortname == 'publickeymodulus' ) {
                                                     echo '<span typeof="rsa:RSAPublicKey" about="#key"><span rel="cert:identity" href="#me"></span> <span property="rsa:modulus" datatype="cert:hex">';
                                                     echo wordwrap(htmlentities($vars['entity']->$shortname, ENT_QUOTES, 'UTF-8'), 40, "<br/>", true) ; // $vars['value'];
                                                     echo '</span></span>';
                                                 } else if ($shortname == 'publickeyexponent' ) {
                                                     echo '<span typeof="rsa:RSAPublicKey" about="#key"> <span property="rsa:public_exponent" datatype="cert:int">';
                                                     echo htmlentities($vars['entity']->$shortname, ENT_QUOTES, 'UTF-8'); // $vars['value'];
                                                     echo '</span></span>';
                                                 } else {
                                                     echo htmlentities($vars['entity']->$shortname, ENT_QUOTES, 'UTF-8'); // $vars['value'];
                                                 }
