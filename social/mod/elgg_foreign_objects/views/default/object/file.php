<?php
	...
	if (get_context() == "search") { 	// Start search listing version 
		...
		if (get_input('search_viewtype') == "gallery") {
			...
			if ($file->foreign) {
				$info = "<p> <a href=\"{$file->link}\">{$title}</a></p>";
			}
			else {
			...
			if ($file->foreign) {
				$icon = "<a href=\"{$file->link}\"><img src='".$file->icon_link."' /></a>";
//				$icon = "<a href=\"{$file->link}\">" . elgg_view("file/icon", array("mimetype" => $mime, 'thumbnail' => $file->icon_link, 'file_guid' => $file_guid, 'size' => 'small')) . "</a>";
			}
			else {
