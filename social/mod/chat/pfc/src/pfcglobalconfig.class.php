...
class pfcGlobalConfig
{
   ...
   * (Default value: '' - means this parameter is automatically calculated)</p>
   */
  var $server_script_url = 'mod/chat/pfc/index.php';
   ...
   */
  var $data_public_url = 'mod/chat/pfc/data/public';

  /**
   * ELGG-Specific: Set to true if you want to add Elgg's wwwroot to <code>data_public_url</code>.
   */
  var $elgg_relative_urls = true;
  ...
  {
    global $CONFIG;
    // Add elgg wwwroot if its set up
    if ($this->elgg_relative_urls) {
        $this->data_public_url = $CONFIG->wwwroot . $this->data_public_url;
    }
    $this->server_script_url = $CONFIG->wwwroot . $this->server_script_url;
