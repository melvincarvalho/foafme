...
foreach ($knows as $k => $v) {
    $auth = new Authentication_AgentARC($GLOBALS['config'], $v['webid']);
    $authAgent = $auth->getAgent();
    $a1 = replace_with_rss($authAgent['holdsAccount']);
    $a2 = replace_with_rss($authAgent['accountProfilePage']);
    $a3 = (array)$authAgent['weblog'];
